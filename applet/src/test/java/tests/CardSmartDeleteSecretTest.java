...
public class CardSmartDeleteSecretTest extends BaseTest {
    ...
    public void deleteSecretFromCard() throws Exception {
        ...
        CommandAPDU cmd = new CommandAPDU(0xC0, 0x22, 0x00, 0x00, data);
        ...
        cmd = new CommandAPDU(0xC0, 0x25, 0x00, 0x00, secretData);
        ...
        cmd = new CommandAPDU(0xC0, 0x26, 0x00, 0x00, name);
        responseAPDU = card.transmit(cmd);
        Assertions.assertNotNull(responseAPDU);
        Assertions.assertEquals(0x9000, responseAPDU.getSW());

        /* Get names on card */
        cmd = new CommandAPDU(0xC0, 0x20, 0x00, 0x00);
        responseAPDU = card.transmit(cmd);
        Assertions.assertNotNull(responseAPDU);
        Assertions.assertEquals(0x9000, responseAPDU.getSW());
    ...
    public void deleteSecretFromCard_noAuthentication() throws Exception {
        ...
        cmd = new CommandAPDU(0xC0, 0x22, 0x00, 0x00, data2);
        responseAPDU = card.transmit(cmd);
        Assertions.assertNotNull(responseAPDU);
